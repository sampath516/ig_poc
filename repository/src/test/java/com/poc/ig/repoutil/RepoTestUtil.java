...
public class RepoTestUtil {
	...
	public static List<User> createUsers(String tenantName, String organization, String manager, int start, int end) {
		...
		for(int i=start; i<=end; i++) {	
			User user = new User(prefix+i+"ExternalId", prefix+i+"Name", prefix+i+"FName", prefix+i+"LName", prefix+i+"Name@gmail.com", manager, organization);
			users.add(user);
		}
		...
		ResponseEntity<CreateUserResponse> userOut = restClient.exchange(uri, HttpMethod.POST, userReq, CreateUserResponse.class);
		Assert.assertEquals(HttpStatus.CREATED, userOut.getStatusCode());
		Assert.assertTrue(userOut.hasBody());
		Assert.assertNotNull(userOut.getBody());
	...
	public static void deleteApplication(String tenantName, String appExternalId) {
		String uri = new StringBuilder(tenantBaseUri).append("/").append(tenantName).append("/applications/").append(appExternalId).toString();
